...
{
    ros::init(argc, argv, "wb_cart_imp_controller");
    ros::NodeHandle nh("~");

    int controller_frequency_hz = 1000;
    nh.param("controller_frequency_hz", controller_frequency_hz, controller_frequency_hz);

    WBCartesianImpedanceController::ControllerManager controller_manager;

    if (!controller_manager.init(argc, argv))
    {
        ROS_ERROR("ControllerManager: initialization failed.");
        return -1;
    }

    if (!controller_manager.starting())
    {
        ROS_ERROR("ControllerManager: starting phase failed.");
        return -1;
    }
    ...
    while (ros::ok() && update_state)
    {
        ...
        if (elapsed_time > controller_period)
        {
            ROS_WARN_STREAM_ONCE(
                "ControllerManager: the controller period is set to " <<
    ...
    ROS_WARN("ControllerManager: ROS stopped controller execution.");
    return 0;
}
